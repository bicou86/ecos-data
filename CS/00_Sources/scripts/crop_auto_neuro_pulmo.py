...
# Scan automatique : toutes les .jpg dont la dimension max >= 2000px sont
# considérées comme des photos cliniques haute-résolution. Les schémas et
# images composites (généralement < 1500px) sont ignorés.
MIN_DIMENSION = 2000
...
def discover_targets():
    ...
    for f in sorted(SOURCE_DIR.iterdir()):
        if f.suffix.lower() != ".jpg" or f.name.startswith("_"):
            continue
        try:
            with Image.open(f) as im:
                if max(im.size) >= MIN_DIMENSION:
...
TARGETS = None  # discovered at runtime
...
if __name__ == "__main__":
    ...
    print(f"{len(targets)} photo(s) à traiter")
    for name in targets:
        process(name)
